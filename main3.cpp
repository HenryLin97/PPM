...
void ConstructScene()
{
    ...
    //s13->Rotate(1, 1);
	s13->Scale(4);
	s13->Translate(vector3d(30, 0, 90));
...
int main(int argc, char *argv[]) {
	...
	Ray cam(vector3d(50,48,295.6), vector3d(0,-0.042612,-1).norm());
	...
	for (int y=0; y<h; y++){
		...
		for (int x=0; x<w; x++) {
			...
			trace(Ray(cam.R0 + d * 140, d.norm()), 0, true, vector3d(), vector3d(1, 1, 1),0);
	...
	FILE* f = fopen("image9.ppm","w"); fprintf(f,"P3\n%d %d\n%d\n",w,h,255);
	for(int i = 0; i< w * h; i++) 
	{
